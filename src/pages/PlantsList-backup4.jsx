...
class PlantsList extends React.Component {
  constructor(props) {
    ...
    // console.log(this.props.showFavorites);
    // console.log(this.props.searchTerm);
    this.state = {
      plants: this.props.plants,
      plantsRefined: this.props.plants,
      searchTerm: this.props.searchTerm ? decodeURI(this.props.searchTerm) : [],
      items: [],
      start: 0,
  ...
  componentDidMount() {
    ...
    if (this.state.showFavorites) {
      ...
      // this.setState({
      //   plantsFavorite: activePlants
      // });
    } else if (this.state.searchTerm.length) {
      activePlants = []
      console.log(activePlants);
      let term = this.state.searchTerm //[0].term
      plants.forEach(item => {
        // console.log(item.name);
        // console.log(term);
        // console.log(item.name.includes(term));
        if (item.name.includes(term)) activePlants.push(item)
        if (item.name.includes(capitalizeFirstLetter(term))) activePlants.push(item)
        else if (item.taxa.genus && item.taxa.genus.includes(term)) activePlants.push(item)
        else if (item.taxa.commonName && (item.taxa.commonName.includes(term) || item.taxa.commonName.includes(capitalizeFirstLetter(term)))) activePlants.push(item)
        else if (item.taxa.commonAlts && (item.taxa.commonAlts.includes(term) || item.taxa.commonAlts.includes(capitalizeFirstLetter(term)))) activePlants.push(item)
  ...
  changeFilter(activeFilters) {
    ...
    if (activeFiltersCount === 0) activePlants = this.state.plants
    // console.log(activePlants);
    this.setState({
      plantsRefined: activePlants,
      items: activePlants.slice(0, this.scrollCount),
  ...
  fetchMoreData = () => {
    ...
    let activePlants = this.state.plantsRefined
    ...
    // setSearch({ page: this.state.page});
  };
  ...
  render() {
    ...
    // console.log(this.state.plants.length);
    // console.log(this.state.plantsRefined.length);
    let activePlants = this.state.plants
    if (this.state.plants.length !== this.state.plantsRefined.length) activePlants = this.state.plantsRefined
    // console.log(activePlants);
    return (
      <div className="row">
        ...
        <div id="plantGrid" className="col-12 col-md-8 col-lg-9">
          {this.state.showFavorites ? (<div className="text-start ml-2 mr-2 page-title"><h2>Favorites</h2></div>) : null }
          <div className="text-end ml-2 mr-2 plant-count">{activePlants.length} items</div>
          {activePlants.length === 0 ? (<p className="mt-5 text-center">No Plants Match</p>) : null}
          <InfiniteScroll
